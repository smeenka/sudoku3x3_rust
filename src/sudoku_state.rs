...
impl SudokuState {
    ...
    pub fn resolve_step( &mut self, board:&SudokuBoard) -> GameState{
        ...
        match  board.resolve_step() {
            ...
            Err(m)  => self.game_state = GameState::Error,
        }
        self.do_count(board);
        if self.curr_count == CELL_SIZE * CELL_SIZE {
            println!("Bingo!");
            self.game_state = GameState::Resolved;
        }
...
pub fn print_layout(row_col_square: &dyn RowColSquare) {
...
pub fn reduce_square(row_col_square: &dyn RowColSquare) -> Result<usize, String> {
